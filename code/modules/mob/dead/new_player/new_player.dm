...
/mob/dead/new_player/proc/create_character(transfer_after)
	...
	var/fra = CONFIG_GET(flag/force_random_appearances)
	if(!frn) //need to check individually so it isn't clobbered by the other not being enabled
		frn = is_banned_from(ckey, "Appearance") //appearance bans ban you from both
		if(QDELETED(src))
			return
	if(!fra)
		fra = is_banned_from(ckey, "Appearance")
		if(QDELETED(src))
			return
	if(fra)
