...
//Get this fucking fetishcontent out of here, even the nu-tg playerbase hates it, fucking delete this
...
/mob/living/carbon/human/proc/update_body_parts_head_only()
	...
	if(HD && !(has_trait(TRAIT_HUSK)))
		...
		if(!(NOEYESPRITES in dna.species.species_traits))
			...
			if(!E)
				eye_overlay = mutable_appearance('icons/mob/human_face.dmi', dna?.species ? dna.species.mutant_eye_missing_state : "eyes_missing", -BODY_LAYER)
			else
