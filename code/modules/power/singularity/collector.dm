...
/obj/machinery/power/rad_collector/examine(mob/user)
	...
	if(active)
		if(!bitcoinmining)
			...
			to_chat(user, "<span class='notice'>[src]'s display states that it has stored a total of <b>[stored_energy*RAD_COLLECTOR_MINING_CONVERSION_RATE]</b>, and producing [RAD_COLLECTOR_OUTPUT*RAD_COLLECTOR_MINING_CONVERSION_RATE] research points per minute.</span>")
	else
		if(!bitcoinmining)
