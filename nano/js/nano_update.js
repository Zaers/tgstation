...
NanoUpdate = function () 
{
	...
	// _canClick can be used to disable clicks for a short period after each click (to avoid mis-clicks)
	var _canClick = true;
		...
		NanoUpdate.addAfterUpdateCallback(function (updateData) {
			...
			$('.linkActive').on('click', function (event) {
				...
				if (href != null && _canClick)
				{
					if (updateData['ui']['status'] == 2)
					{						
						$(this).oneTime(100, 'linkPending', function () {
...
{
	NanoUpdate.init(); //so we need to delay the starting gun a bit	
});
